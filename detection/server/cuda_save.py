...
def get_answer(question, context):
    ...
    Use the following pieces of context to determine if the statement is true or false, and explain why. 
    If you don't have enough information, say so and explain why. Use the format Answer: *the actual answer* Explanation: *the actual explanation*
...
def process_input(input_text):
    ...
    full_query_content, full_query_link = search_wikipedia(input_text)
    ...
    for entity in entities:
        ...
        if content and content not in retrieved_articles:
            ...
            logger.info(f"Article for '{content}' already utilized")
        else:
    ...
    formatted_result = {
        ...
        "message": answer,
        "links": [{"title": link.split('/')[-1].replace('_', ' '), "link": link} for link in article_links]
    }
...
def is_false():
    ...
    try:
        ...
        is_false = "false" in result["message"].lower() or "not true" in result["message"].lower()
